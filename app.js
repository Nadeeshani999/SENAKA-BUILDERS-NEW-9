...
function renderKPIs(containerId, cards) {
  ...
  el.innerHTML=cards.map((c,i)=>{
    ...
    // delta: detect sign for color
    let deltaHtml='';
    if(c.delta){
      const isNeg=String(c.delta).includes('-');
      const isPos=!isNeg && (String(c.delta).includes('+')||String(c.delta).match(/\d/));
      const dClass=isNeg?'kpi-delta-neg':isPos?'kpi-delta-pos':'kpi-delta-neutral';
      const arrow=isNeg?'↓':'↑';
      deltaHtml=`<div class="kpi-delta ${dClass}">${arrow} ${c.delta}</div>`;
    }
    return `<div class="kpi-card" style="--kpi-rgb:${rgb};--stagger:${i}" data-canvas="${canvasId}">
  ...
  setTimeout(()=>{
    cards.forEach((c,i)=>{
      ...
      const col=c.color||'primary';
      const rgb=KPI_RGB[col]||'0,200,255';
      // Force canvas to fill its card width before drawing
      ...
      const data=c.sparkData||generateFakeSparkData(c.rawVal||0);
...
function generateFakeSparkData(finalVal) {
  // Generate 12 plausible data points leading to finalVal
  const points=[];
  let v=finalVal*(0.6+Math.random()*0.2);
  for(let i=0;i<12;i++){
    v+=( (finalVal-v)*0.3 + (Math.random()-0.45)*finalVal*0.08 );
...
function drawSparkline(canvas, data, rgb) {
  ...
  // getBoundingClientRect is the most reliable — works even during animation
  const card = canvas.closest('.kpi-card');
